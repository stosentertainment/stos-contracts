...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("STOS Lottery Ticket", "SLT") {}
...
contract Lottery is LotteryOwnable {
    ...
    // issueIndex => adminShareAmount
    mapping(uint256 => uint256) public adminShare;
    address public adminShareDepositAddress;

    uint256 public issueIndex = 0;
    uint256 public totalAddresses = 0;
    uint256 public totalAmount = 0;
    uint256 public lastTimestamp;
    uint256 public multiBuyLimit = 40;
    ...
    constructor(
        ...
        adminShareDepositAddress = _adminAddress;
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        uint256 _adminShare = totalAmount
            .mul(
            uint256(100).sub(allocation[0]).sub(allocation[1]).sub(
                allocation[2]
            )
        )
            .div(100);

        adminShare[issueIndex] = _adminShare;
        stos.safeTransfer(adminShareDepositAddress, _adminShare);
        drawingPhase = false;
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        stos.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        require(_price >= minPrice, "price must above minPrice");
        require(
            _numbers.length <= multiBuyLimit,
            "can only buy 40 ticket at once"
        );
        ...
        stos.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            stos.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            stos.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function setAdminShareDepositAddress(address _adminShareDepositAddress)
        public
        onlyOwner
    {
        adminShareDepositAddress = _adminShareDepositAddress;
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        stos.safeTransfer(address(msg.sender), _amount);
    ...
    // Set the max lottery numbers
    function setMaxNumber(uint8 _maxNumber) external onlyAdmin {
        maxNumber = _maxNumber;
    }

    // Set the minimum price for one ticket
    function setMultiBuyLimit(uint8 _limit) external onlyAdmin {
        multiBuyLimit = _limit;
    }
